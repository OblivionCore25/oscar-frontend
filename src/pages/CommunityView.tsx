    ...
    <div className="flex flex-col h-full bg-[#0a0a12]">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="bg-[#12121a] p-6 rounded-xl border border-[#2a2a35] shadow-sm flex items-start gap-4">
            <div className="p-3 bg-emerald-100 rounded-lg text-emerald-400 shrink-0">
              <Box className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-gray-200 mb-1">Subsystem Drift Detection</h2>
          ...
          {error && (
            <div className="bg-red-900/30 border border-red-900/50 p-6 rounded-xl text-center">
              ...
              <p className="text-red-400 font-medium">Failed to compute Louvain graphs.</p>
            </div>
          )}
          ...
          {sortedCommunities.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {sortedCommunities.map(([cid, methods]) => {
                ...
                return (
                  <div key={cid} className="bg-[#12121a] border border-[#2a2a35] rounded-xl shadow-sm flex flex-col h-[400px]">
                    <div className="p-4 border-b border-white/5 shrink-0 bg-[#0a0a12]/50 rounded-t-xl">
                      <div className="flex justify-between items-center mb-1">
                        <h3 className="font-bold text-gray-200">Community #{cid}</h3>
                        <span className="bg-emerald-100 text-emerald-400 font-bold px-2 py-0.5 rounded-md text-xs">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2">
                      {methods.map(m => (
                        <div key={m.method.id} className="p-2 bg-[#0a0a12] border border-white/5 rounded text-[11px] font-mono hover:border-emerald-200 transition-colors">
                          <p className="font-bold text-gray-300 truncate">{m.method.name}</p>
                          <p className="text-gray-500 mt-0.5 truncate">{m.method.file_path?.split('/').pop()}:{m.method.start_line}</p>
                        </div>
                      ))}
                    </div>
                  </div>
