...
export default function MethodGraphViewer() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a12]">
      <header className="bg-[#12121a] border-b border-[#2a2a35] px-6 py-4 flex items-center justify-between shrink-0 shadow-sm z-10">
      ...
      <main className="flex-1 relative overflow-hidden bg-[#2a2a35] flex">
        <div className={`flex-1 relative transition-all duration-300 ${selectedNodeId ? 'mr-80' : ''}`}>
          {isMetaRedirect && originalSlug && !bannerDismissed && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 bg-amber-900/30 border border-amber-200 text-amber-800 px-4 py-2 flex items-center gap-3 rounded-lg shadow-sm text-xs animate-in slide-in-from-top-4">
              <Network className="w-4 h-4 text-amber-400 shrink-0" />
              <span>
                <strong>Meta-Package Auto-Resolved:</strong> <span className="font-mono bg-amber-900/40 px-1 rounded">{originalSlug}</span> contained no AST nodes. You are viewing its structural core <span className="font-mono bg-amber-900/40 px-1 rounded">{project}</span> instead.
              </span>
              <button
                onClick={() => setBannerDismissed(true)}
                className="ml-1 p-0.5 rounded hover:bg-amber-200 text-amber-400 hover:text-amber-800 transition-colors shrink-0"
                aria-label="Dismiss"
              >
          ...
          {!project && (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center h-full">
              ...
              <button onClick={() => navigate('/methods')} className="mt-4 px-4 py-2 bg-[#12121a] border rounded shadow-sm hover:bg-[#1a1a2e]">Back to Explorer</button>
            </div>
          )}
          ...
          {project && isLoading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0a0a12]/80 backdrop-blur-sm z-20">
              ...
              <p className="text-gray-500 font-medium">Booting WebGL graph topology...</p>
            </div>
          )}

          {project && error && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0a0a12]/80 z-20">
              <div className="bg-[#12121a] p-8 rounded-xl shadow-sm border border-red-900/50 text-center max-w-md">
                <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
                <h2 className="text-lg font-bold">Topology Render Failed</h2>
                <p className="text-red-500 mt-2 font-mono text-xs bg-red-900/30 rounded p-2">{(error as any).message}</p>
                <p className="text-gray-500 mt-3 text-xs">
                  Looking for project: <span className="font-mono text-gray-500">{project}</span>
                </p>
                <div className="flex gap-2 mt-5 justify-center">
                  <button onClick={() => navigate('/methods')} className="px-4 py-2 border rounded hover:bg-[#1a1a2e] text-sm">Browse Projects</button>
          ...
          {project && data && !isLoading && !error && data.nodes.length === 0 && (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center h-full bg-[#0a0a12] z-10">
              <AlertCircle className="w-12 h-12 text-slate-400 mb-4" />
              <h2 className="text-xl font-bold text-slate-300">No Computational Topology Found</h2>
              ...
              <button onClick={() => navigate('/methods')} className="mt-5 px-4 py-2 bg-[#12121a] border border-slate-300 rounded shadow-sm hover:bg-[#0a0a12] font-medium">Browse Projects</button>
            </div>
          )}
        </div>
        ...
        {selectedNodeId && selectedNodeDetails && (
          <div className="absolute top-0 right-0 bottom-0 w-80 bg-[#12121a] border-l border-[#2a2a35] shadow-xl z-20 overflow-y-auto flex flex-col animate-in slide-in-from-right-8 duration-200">
            <div className="p-4 border-b border-white/5 flex justify-between items-start bg-[#0a0a12] sticky top-0">
               <div className="min-w-0 pr-2">
                 <h2 className="font-bold text-gray-100 text-sm truncate" title={selectedNodeDetails.name}>
            ...
            <div className="p-4 flex-1 space-y-6 bg-[#12121a]">
              <div>
                <h3 className="text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-2">Metrics</h3>
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-[#0a0a12] p-2.5 rounded-lg border border-white/5">
                    <span className="text-[10px] text-gray-500 block mb-1 leading-none font-medium">
                      <MetricTooltip metric={METHOD_METRICS.methodComplexity}>Complexity</MetricTooltip>
                    </span>
                    <span className={`font-mono text-sm leading-none font-bold ${selectedNodeDetails.complexity > 10 ? 'text-amber-400' : 'text-slate-300'}`}>{selectedNodeDetails.complexity || 0}</span>
                  </div>
                  <div className="bg-[#0a0a12] p-2.5 rounded-lg border border-white/5">
                    <span className="text-[10px] text-gray-500 block mb-1 leading-none font-medium">
                      <MetricTooltip metric={METHOD_METRICS.methodCentrality}>Centrality</MetricTooltip>
                    </span>
                    <span className="font-mono text-sm leading-none font-bold text-indigo-400">{(selectedNodeDetails.betweenness_centrality || 0).toFixed(4)}</span>
                  </div>
                  <div className="bg-[#0a0a12] p-2.5 rounded-lg border border-white/5">
                    <span className="text-[10px] text-gray-500 block mb-1 leading-none font-medium">
                      <MetricTooltip metric={METHOD_METRICS.methodFanIn}>Fan In</MetricTooltip>
                    </span>
                    <span className="font-mono text-sm leading-none font-bold text-slate-300">{selectedNodeDetails.fan_in || 0}</span>
                  </div>
                  <div className="bg-[#0a0a12] p-2.5 rounded-lg border border-white/5">
                    <span className="text-[10px] text-gray-500 block mb-1 leading-none font-medium">
                      <MetricTooltip metric={METHOD_METRICS.methodFanOut}>Fan Out</MetricTooltip>
                    </span>
                    <span className="font-mono text-sm leading-none font-bold text-slate-300">{selectedNodeDetails.fan_out || 0}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">
                ...
                <div className="bg-purple-50 p-4 rounded-xl border border-purple-100">
                  <span className="text-4xl font-black text-purple-400 block mb-2 leading-none">{selectedNodeDetails.blast_radius || 0}</span>
                  <span className="text-[11px] font-medium text-purple-400/80 leading-snug block">Transitively affected methods downstream when modified.</span>
                </div>
              </div>
              ...
              {selectedNodeDetails.community_id !== undefined && selectedNodeDetails.community_id !== null && (
                <div>
                  <h3 className="text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-2">
                    <MetricTooltip metric={METHOD_METRICS.communities}>Architecture</MetricTooltip>
                  </h3>
