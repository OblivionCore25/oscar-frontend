...
export const SUPPLY_CHAIN_METRICS: Record<string, MetricInfo> = {
  ...
  betweenness: {
    ...
    definition: 'The fraction of all shortest paths in the local transitive dependency subgraph that pass through this package. Measures its "bridge" or gatekeeper role within its own tree.',
    formula: 'CB(v) = Σ σ(s,t|v) / σ(s,t) for all pairs s,t',
    whyItMatters: 'A package with high betweenness acts as a critical bridge — removing it could disconnect large portions of the local dependency tree.',
    citation: 'Freeman, L. C. (1977). A Set of Measures of Centrality Based on Betweenness.',
    glossaryAnchor: 'betweenness',
  },
  closeness: {
    name: 'Closeness Centrality',
    definition: 'The inverse of the average shortest-path distance from this node to all other reachable nodes in its local transitive subgraph.',
    formula: 'CC(v) = (N-1) / Σ d(v,u) for all reachable u',
    whyItMatters: 'High closeness means a failure at this node propagates quickly to the rest of the local dependency tree — it is structurally "close" to everything.',
